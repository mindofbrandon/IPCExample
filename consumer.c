...
int main(void)
{
    ...
    shm_fd = shm_open(name, O_RDONLY, 0666); // only want consumer to read from shared memory
    if (shm_fd == -1) {
    ...
    if (shm_unlink(name) == -1) {   // this is the last process to work with shared region, unlink shared region from file system
                                    // I believe this is the reason where if you run producer twice, the memory will not be clean
                                    // because it did not unlink
